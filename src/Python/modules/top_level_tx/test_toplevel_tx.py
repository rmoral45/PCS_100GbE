...
AM_OUT_FILE = './dump/amInsert_output.txt'
AM_BLOCK_PERIOD = 16383
N_LANES = 20
def main():
...
#############  read output from am_insertion mods
        ...
        with open(AM_OUT_FILE) as fd:
                ...
                for data in contents[:-1]:
                        am_blocks.append(data) # CHECK, creo que esto era por el tema del tag  !!!!!!!
                ...
                n_simulated_periods = int(len(am_blocks) /(AM_BLOCK_PERIOD * N_LANES)) # obtengo cantidad de periodos COMPLETOS (division entera)
                am_blocks           = am_blocks[:(n_simulated_periods * AM_BLOCK_PERIOD * N_LANES)]
                masked_blocks       = masked_blocks[:(n_simulated_periods * AM_BLOCK_PERIOD * N_LANES)]
